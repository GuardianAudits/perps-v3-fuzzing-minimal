...
function parsePackageArguments(val, result) {
    const packageMatch = builder_1.PackageReference.isValid(val);
    if (!result && !packageMatch) {
        throw new commander_1.InvalidArgumentError('First argument should be a cannon package name, e.g.: greeter:1.0.0 or greeter:latest@main');
    }
    ...
    if (packageMatch) {
        const pkg = new builder_1.PackageReference(val);
