...
exports.ChainDefinition = void 0;
...
const debugVerbose = (0, debug_1.default)('cannon:verbose:builder:definition');
function validatePackageName(n) {
    if (n.length < 3) {
    ...
    if (lodash_1.default.last(n) == '-' || lodash_1.default.first(n) == '-') {
...
function validatePackageVersion(v) {
    if (v.length > 31) {
        throw new Error('package version must be at most 31 characters long');
    }
}
class ChainDefinition {
    constructor(def, sensitiveDependencies = false) {
        this.cachedLayers = null;
        this.cachedActionDepths = new Map();
        this.dependencyFor = new Map();
        ...
        this.getDependencyTree = lodash_1.default.memoize((node) => {
            ...
            return [lodash_1.default.uniq(allDeps), maxDepth];
        });
        ...
        const actionsDef = lodash_1.default.omit(def, 'name', 'version', 'preset', 'description', 'keywords');
        ...
        for (const [action, data] of Object.entries(actionsDef)) {
            for (const name of Object.keys(data)) {
                ...
                if (actions_1.ActionKinds[action] && actions_1.ActionKinds[action].getOutputs) {
                    const actionOutputs = actions_1.ActionKinds[action].getOutputs(lodash_1.default.get(def, fullActionName), {
    ...
    getName(ctx) {
        const n = lodash_1.default.template(this.raw.name)(ctx);
        validatePackageName(n);
        return n;
    }
    getVersion(ctx) {
        const v = lodash_1.default.template(this.raw.version)(ctx);
        validatePackageVersion(v);
        return v;
    }
    getPreset(ctx) {
        return lodash_1.default.template(this.raw.preset)(ctx) || 'main';
    }
    isPublicSourceCode() {
    ...
    getConfig(n, ctx) {
        ...
        const action = actions_1.ActionKinds[kind];
        if (!action) {
        ...
        (0, actions_1.validateConfig)(action.validate, lodash_1.default.get(this.raw, n));
        return action.configInject({ ...ctx, ...types_1.CannonHelperContext }, lodash_1.default.get(this.raw, n), {
     ...
     */
    async getState(n, runtime, ctx, tainted) {
        const kind = n.split('.')[0];
        if (!actions_1.ActionKinds[kind]) {
        ...
        const objs = await actions_1.ActionKinds[kind].getState(runtime, { ...ctx, ...types_1.CannonHelperContext }, this.getConfig(n, ctx), {
            name: this.getName(ctx),
            version: this.getVersion(ctx),
            currentLabel: n,
        });
        if (!objs) {
            ...
            debugVerbose('creating hash of', objs.map(JSON.stringify));
            return objs.map((o) => crypto_1.default.createHash('md5').update(JSON.stringify(o)).digest('hex'));
        }
    }
    /**
    ...
    computeDependencies(node) {
        if (!lodash_1.default.get(this.raw, node)) {
            const stepName = node.split('.')[0];
            const possibleSteps = lodash_1.default.get(this.raw, stepName);
            if (!possibleSteps) {
        ...
        const deps = lodash_1.default.clone(lodash_1.default.get(this.raw, node).depends || []);
        const n = node.split('.')[0];
        if (!actions_1.ActionKinds[n]) {
            throw new Error(`Unrecognized action type ${n} at [${node}]`);
        }
        if (actions_1.ActionKinds[n].getInputs) {
            ...
            const accessComputationResults = actions_1.ActionKinds[n].getInputs(lodash_1.default.get(this.raw, node), possibleFields, {
                name: '',
                version: '',
                currentLabel: node,
            });
            // Only throw this error if the user hasn't explicitly defined dependencies
            if (this.sensitiveDependencies && accessComputationResults.unableToCompute && !lodash_1.default.get(this.raw, node).depends) {
                throw new Error(`Unable to compute dependencies for [${node}] because of advanced logic in template strings. Specify dependencies manually, like "depends = ['${lodash_1.default.uniq(lodash_1.default.uniq(accessComputationResults.accesses).map((a) => `${this.dependencyFor.get(a)}`)).join("', '")}']"`);
            }
    ...
    get topologicalActions() {
        ...
        return lodash_1.default.sortBy(this.allActionNames, (n) => this.cachedActionDepths.get(n));
    }
    checkAll() {
        const invalidSchema = (0, actions_1.validateConfig)(schemas_1.chainDefinitionSchema, this.raw);
    ...
    getStateLayers() {
        ...
        const layers = {};
        const layerOfActions = new Map();
        const layerDependingOn = new Map();
        ...
        for (const n of actions) {
            ...
            deps = lodash_1.default.sortBy(deps, (d) => -this.cachedActionDepths.get(d));
