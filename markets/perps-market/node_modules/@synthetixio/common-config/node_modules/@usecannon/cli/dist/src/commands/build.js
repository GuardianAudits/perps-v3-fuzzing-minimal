...
const write_1 = require("../write-script/write");
async function build({ provider, def, packageDefinition, upgradeFrom, pkgInfo, getArtifact, getSigner, getDefaultSigner, presetArg, overrideResolver, wipe = false, persist = true, plugins = true, publicSourceCode = false, providerUrl, registryPriority, gasPrice, gasFee, priorityGasFee, writeScript, writeScriptFormat = 'ethers', }) {
    if (wipe && upgradeFrom) {
    ...
    const dump = writeScript ? await (0, write_1.createWriteScript)(runtime, writeScript, writeScriptFormat) : null;
    ...
    if (miscUrl) {
        ...
        if (partialDeploy) {
            ...
            if (chainId == 13370) {
                ...
                console.log((0, chalk_1.gray)('This package can be run locally and cloned in cannonfiles.'));
            }
            else {
            ...
            console.log();
            console.log((0, chalk_1.bold)(`Package data has been stored locally${cliSettings.writeIpfsUrl && ' and pinned to ' + cliSettings.writeIpfsUrl}`));
